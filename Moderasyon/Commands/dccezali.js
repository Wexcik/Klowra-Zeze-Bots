...
module.exports.execute = async (client, message, args, ayar, emoji) => {
    ...
    if (!client.kullanabilir(message.author.id) && !ayar.dcsorumlusu.some(rol => message.member.roles.cache.has(rol))) return message.lineReply(embed.setDescription('`Yeterli Yetkin Bulunmamakta.`')).then(x => x.delete({ timeout: 5000 }));
    ...
    collector.on('collect', async (reaction) => {
        if (reaction.emoji.name == "1️⃣") {
            ...
            message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("7")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle dc ccezalıya atıldı`)).catch();
            client.channels.cache.find(c => c.name === ayar.dccezalılog).send(new MessageEmbed().setColor('f39c12').setDescription(`Engelleyen : ${message.author} \`(${message.author.id})\`\nEngellenen: ${uye} \`(${uye.id})\`\nSebep: ${reason}\nSüre: ${new Date(bitisTarihi).toTurkishFormatDate()} \`(${moment.duration(ms("7d")).format('D [gün,] H [saat,] m [dakika]')})\`\nCeza-i İşlem: Dc Cezalı \`(${cezaNumara})\``)).catch(console.error);
            newPenalty.save();
            msj.delete()
            ...
            message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("14")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle dc ccezalıya atıldı`)).catch();
        ...
        message.lineReply(embed.setDescription(`${uye} üyesi, ${message.author} tarafından **${moment.duration(ms("21")).format('D [gün,] H [saat,] m [dakika]')}** boyunca **${reason}** nedeniyle dc ccezalıya atıldı`)).catch();
        ...
        msj.delete()
            ...
            Penalty.find({ sunucuID: message.guild.id, uyeID: uye.id }).sort({ atilmaTarihi: -1 }).exec(async (err, data) => {
                ...
                message.lineReply(embed.setDescription(`${uye} üyesinin ${message.guild.roles.cache.get(ayar.dccezalırolu).toString()} rolü, ${message.author} tarafından alındı!`)).catch();
                if (client.channels.cache.find(c => c.name === ayar.dccezalılog)) client.channels.cache.find(c => c.name === ayar.dccezalılog).send(new MessageEmbed().setColor("GREEN").setDescription(`${uye} üyesinin ${message.guild.roles.cache.get(ayar.dccezalırolu).toString()} rolü, ${message.author} tarafından alındı!`)).catch();
...
module.exports.configuration = {
    ...
    aliases: ["dcc"],
    usage: 'dcc [üye] [sebep]',
    description: 'Belirtilen üyeyi doğruluk cesaret oyununda cezalandırır.',
    permLevel: 0
};